...
public:
    ...
    void drawGame() {
        ...
        for (int tileNumber = 0; tileNumber < TILE_COUNT; tileNumber++) {
            if (playGround->tiles[tileNumber] != nullptr) {
                DrawDecal({ (float)xCoords[tileNumber], (float)yCoords[tileNumber] }, playGround->tiles[tileNumber]->mainDecal);
            }
        }
        if (tileSelected()) {
            DrawDecal({ (float)xCoords[selectedTileNumber], (float)yCoords[selectedTileNumber] }, playGround->selectedTileDecal);
        }
    }
    bool winCheck() {
    ...
    bool columnMatchCheck(TileType type, int columnNumber) {
        for (int columnTile = columnNumber; columnTile < TILE_COUNT; columnTile += TILE_ROW_COLUMN_COUNT) {
            ...
            if (playGround->tiles[columnTile]->type != type) {
                return false;
            }
        }
        return true;
    ...
    bool adjacentTileCheck(int targetTileNumber) {
        ...
        else if (selectedTileNumber == targetTileNumber + TileDirection::Right) {
